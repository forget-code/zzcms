...
?>
<div class="admintitle"><img src="/image/ico1.gif" width="12" height="12"> 展厅留言</div> 
			...
			case "no";
			    if (jifen=="Yes"){
					...
					$rowuser=mysql_fetch_array($rsuser);
						...
						mysql_query("insert into zzcms_pay (username,dowhat,RMB,mark,sendtime) values('".@$_COOKIE['UserName']."','查看展厅留言','-".jf_lookmessage."','<a href=ztliuyan_show.php?id=$id>$id</a>','".date('Y-m-d H:i:s')."')");//写入冲值记录 
			       		mysql_query("update zzcms_guestbook set looked=1 where id='$id'");
						echo showlx($dlsname,$tel,$email);
						}else{
