...
mysql_query("update zzcms_specialclass set xuhao='$xuhao' where classid='".$row['classid']."'");
...
mysql_query("update zzcms_specialclass set xuhao='$xuhaos' where classid='".$rown['classid']."'");
}
}
}
...
if ($bigclassid<>"") {
	mysql_query("delete from zzcms_specialclass where parentid=" . $bigclassid. "");
	mysql_query("delete from zzcms_specialclass where classid=" . $bigclassid. "");
}
//mysql_close($conn);      
...
if ($smallclassid<>"") {
	mysql_query("delete from zzcms_specialclass where classid=" . $smallclassid. "");
}
//mysql_close($conn);      
...
for($i=0; $i<count($_POST['classname']);$i++){
	...
	$row=mysql_num_rows($rs);
		if (!$row) {
		mysql_query("insert into zzcms_specialclass (classname,parentid,isshowforuser,isshowininfo)values('$classname',0,'$isshowforuser','$isshowininfo')");	
		}
	}
}	
$rsbcid=mysql_query("select classid from zzcms_specialclass where classname='".$classname."'");
...
if ($action=="add") {
	...
	$row=mysql_num_rows($rs);
		if (!$row) {
		mysql_query("insert into zzcms_specialclass (parentid,classname,isshowforuser,isshowininfo)values('$bigclassid','$classname','$isshowforuser','$isshowininfo')");
		}
	}
	}	
...
$discription=$classname;
	...
	if ($FoundErr==0){
	mysql_query("update zzcms_specialclass set classname='$classname',isshowforuser='$isshowforuser',isshowininfo='$isshowininfo',title='$title',keyword='$keyword',discription='$discription' where classid=" .$classid."");
	
		if ($classname<>$oldclassname) {//类名改变的情况下
			mysql_query("Update zzcms_special set bigclassname='" . $classname . "'  where bigclassid=" . $classid . " ");	
		}	
		//mysql_close($conn);
...
$discription=$classname;
	...
	if ($FoundErr==0) {
	mysql_query("update zzcms_specialclass set parentid='$bigclassid',classname='$classname',isshowforuser='$isshowforuser',isshowininfo='$isshowininfo',title='$title',keyword='$keyword',discription='$discription' where classid=" .$classid."");
			if ($bigclassid<>$oldbigclassid) {//小类别改变所属大类情况下
				mysql_query("Update zzcms_special set bigclassid=" . $bigclassid . " where bigclassid=" . $oldbigclassid . " and smallclassid=" . $classid . " ");
				mysql_query("Update zzcms_special set bigclassname=(select classname from zzcms_specialclass where classid=".$bigclassid.") where bigclassid=" . $bigclassid . " and smallclassid=" . $classid . " ");	
			}
			if ($classname<>$oldclassname) {//小类名改变的情况下
				mysql_query("Update zzcms_special set smallclassname='" . $classname . "'  where bigclassid=" . $bigclassid . " and smallclassid=" . $classid . " ");
			}
			//mysql_close($conn);
