...
$str=$title . "|||" .$bigclassid . "|||".$elite . "|||" . $numbers . "|||" . $orderby ."|||" . $titlenum ."|||" . $row . "|||" . $start . "|||" . $mids . "|||" . $ends;
fputs($fp,$str);
fclose($fp);
...
if ($action=="del") {
checkadminisdo("label");
$f="../template/".siteskin."/label/zhshow/".trim($_POST["title"]).".txt";
	if (file_exists($f)){
	unlink($f);
	}else{
