...
if(!empty($_POST['id'])){
	...
	$row = mysql_fetch_array($rs);
		if ($row['passed']=='0'){
		mysql_query("update zzcms_wangkan set passed=1 where id ='$id'");
		}else{
		mysql_query("update zzcms_wangkan set passed=0 where id ='$id'");
		}
	}
}else{
...
?>
    <tr class="bgcolor1" onMouseOver="fSetBg(this)" onMouseOut="fReBg(this)"> 
	  ...
	  <?php
	  echo "<a href=?b=".$row['bigclassid'].">"; 
	  $rsn=mysql_query("select bigclassname from zzcms_wangkanclass where bigclassid='".$row["bigclassid"]."'");
