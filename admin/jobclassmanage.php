...
mysql_query("update zzcms_jobclass set xuhao='$xuhao' where classid='".$row['classid']."'");
...
mysql_query("update zzcms_jobclass set xuhao='$xuhaos' where classid='".$rown['classid']."'");
}
}
}
...
if ($bigclassid<>"") {
	mysql_query("delete from zzcms_jobclass where parentid=" . $bigclassid. "");
	mysql_query("delete from zzcms_jobclass where classid=" . $bigclassid. "");
}
//mysql_close($conn);      
...
if ($smallclassid<>"") {
	mysql_query("delete from zzcms_jobclass where classid=" . $smallclassid. "");
}
//mysql_close($conn);      
...
for($i=0; $i<count($_POST['classname']);$i++){
	...
	$row=mysql_num_rows($rs);
		if (!$row) {
		mysql_query("insert into zzcms_jobclass (classname,parentid)values('$classname',0)");	
		}
	}
}	
$rsbcid=mysql_query("select classid from zzcms_jobclass where classname='".$classname."'");
...
if ($action=="add") {
    ...
    $classname=($_POST['classname'][$i]);
		...
		$row=mysql_num_rows($rs);
			if (!$row) {
			mysql_query("insert into zzcms_jobclass (parentid,classname)values('$bigclassid','$classname')");
			}
		}
	}	
...
$oldclassname=trim($_POST["oldclassname"]);
	...
	if ($FoundErr==0){
	mysql_query("update zzcms_jobclass set classname='$classname' where classid=" .$classid."");
...
$oldclassname=trim($_POST["oldclassname"]);
	...
	if ($FoundErr==0) {
	mysql_query("update zzcms_jobclass set parentid='$bigclassid',classname='$classname' where classid=" .$classid."");
			if ($bigclassid<>$oldbigclassid) {//小类别改变所属大类情况下
				...
				mysql_query("Update zzcms_job set bigclassname=(select classname from zzcms_jobclass where classid=".$bigclassid.") where bigclassid=" . $bigclassid . " and smallclassid=" . $classid . " ");	
			}
