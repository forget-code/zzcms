...
$rs=mysql_query("select * from zzcms_zh where editor='".$_COOKIE["UserName"]."'");
$row=mysql_num_rows($rs);
if (!$row){
$c=0;
}else{
...
if ($a+$c==0){     
	echo "<script>alert('您尚未发布信息，不能抢占广告位！请先发布".channelzs."信息。');";
	echo "location.replace('zsadd.php')</script>";	
}else{
...
<div class="border2" style="padding:8px;margin:1px"> 注：抢占方式：先抢先得；每个用户只限抢占一个广告位。抢占成功后，广告位占用期限为 <b><?php echo showadvdate?></b> 天。到期后可重新参与抢占。
</div>
...
echo "<div><input name=\"Submit\" type=\"button\"  disabled value=\"收费广告位，不可抢占\"></div>";
      ...
      <form name="form1" method="post" action="">
		...
		?>
		<div><input name="Submit" style="width:160px" type="button"  disabled value="<?php echo number_format((24*3600*showadvdate-(time()-strtotime($row['sendtime'])))/(24*3600),1,'.','') ?>天后可抢占"></div>
		<?php
		}
}		
		?>
