...
?>
<div class="admintitle"><img src="/image/ico1.gif" width="12" height="12"> <?php echo $f_array[6]?></div> 
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="bgcolor">
        ...
        <div class="admintitle"><img src="/image/ico1.gif" width="12" height="12"> 
          <?php echo $f_array[10]?></div>
		  <div class="bgcolor" style="padding:1px">
 <?php
         switch  (check_user_power("look_dls_liuyan")){
			case "yes" ;
			mysql_query("update zzcms_guestbook set looked=1 where id='$id'");
			...
			case "no";
			    if (jifen=="Yes"){
					...
					$rowuser=mysql_fetch_array($rsuser);
						...
						mysql_query("insert into zzcms_pay (username,dowhat,RMB,mark,sendtime) values('".@$_COOKIE['UserName']."','".$f_array[12]."','-".jf_lookmessage."','<a href=ztliuyan_show.php?id=$id>$id</a>','".date('Y-m-d H:i:s')."')");//写入冲值记录 
			       		mysql_query("update zzcms_guestbook set looked=1 where id='$id'");
						echo showlx($dlsname,$tel,$email);
						}else{
