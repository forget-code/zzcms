...
?>
	...
	<div class="titlebig" >您是首次用QQ登陆，请按提示完成绑定</div>
	<?php
		if ($_POST["isuser"]==1 && $_POST["action"]=="step2") {
				...
				//登陆
				mysql_query("UPDATE zzcms_user SET showlogintime = lastlogintime where qqid='".$qqid."'");//更新上次登陆时间
				mysql_query("UPDATE zzcms_user SET showloginip = loginip where qqid='".$qqid."'");//更新上次登陆IP
				mysql_query("UPDATE zzcms_user SET logins = logins+1 where qqid='".$qqid."'");
				mysql_query("UPDATE zzcms_user SET loginip = '".getip()."' where qqid='".$qqid."'");//更新最后登陆IP
					if (strtotime(date("Y-m-d H:i:s"))-strtotime($row['lastlogintime'])>86400){
					mysql_query("UPDATE zzcms_user SET totleRMB = totleRMB+".jf_login." WHERE qqid='".$qqid."'");//登陆时加积分
					}
				mysql_query("UPDATE zzcms_user SET lastlogintime = '".date('Y-m-d H:i:s')."' WHERE qqid='".$qqid."'");//更新最后登陆时间
	...
	<form name="form2" id="form2" method="post" action="">
        ...
        <table width="100%" height="100" border="0" cellpadding="10" cellspacing="0">
          <tr> 
        <td width="31%">&nbsp;</td>
          <td width="69%" class="bigbigword">
<input type="radio" name="isuser" id="isuseryes" value="1" onClick="this.form.submit()"/> 
...
<form name="form1" id="form1" method="post" action="" onSubmit="return CheckForm()">
        <table width="100%" height="100" border="0" cellpadding="10" cellspacing="0">
          ...
          <tr> 
            ...
            <td><input type="submit" name="Submit2" value="绑定登陆QQ" /> <input name="action" type="hidden" id="action6" value="step2" /> 
...
<form name="form2" id="form2" method="post" action="/reg/<?php echo getpageurl3("userreg")?>">
        <table width="100%" height="100" border="0" cellpadding="10" cellspacing="0">
          <tr> 
            <td width="38%">&nbsp;</td>
            <td width="62%">请先注册成为本站会员后再用QQ登陆</td>
          </tr>
          <tr> 
            <td>&nbsp;</td>
...
setcookie("UserName",$row['username'],time()+3600*24,"/");
setcookie("PassWord",$row['password'],time()+3600*24,"/");
echo '<span class=bigbigword>登陆成功，正在跳转，请稍候……</span><br>';
}
