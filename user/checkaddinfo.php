...
$ErrMsg="";
$FoundErr=0;
$rs=mysql_query("select passed,usersf,username from zzcms_user where username='".$_COOKIE["UserName"]."' ");
...
//if ($tablename=="zzcms_main" || $tablename=="zzcms_licence") {
	...
	//}	
//}
$rs=mysql_query("select addinfo_number,addinfototle_number from zzcms_usergroup where groupid=(select groupid from zzcms_user where username='".$_COOKIE["UserName"]."')");
$row=mysql_fetch_array($rs);
if ($row){
...
$rs=mysql_query("select id from ".$tablename." where editor='".$_COOKIE["UserName"]."'");
$row=mysql_num_rows($rs);
